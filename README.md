...
- **Ports** — web `3000`, api `4000`, postgres `5437` (dev ใช้ web proxy `/api` → api จึงไม่มีปัญหา CORS/cookie; postgres เผย port 5437 เพื่อเลี่ยง PostgreSQL ที่ติดตั้งไว้บนเครื่อง port 5432)
...
DATABASE_URL='postgresql://postgres:postgres@localhost:5437/dev' 

pnpm install

pnpm docker:up                    # postgres 17 บน localhost:5437
